...
export class AuthController {
  ...
  handleGoogleLogin: RequestHandler = asyncHandler(
    async (req: Request, res: Response) => {
      ...
      const updatedUser = await this.userServices.updateUsersRefreshToken(
        newUser.id,
        refreshToken
      );
  ...
  handleRegister: RequestHandler = asyncHandler(
    async (req: Request, res: Response) => {
  ...
  handleLogin:RequestHandler = asyncHandler(async(req:Request, res:Response)=>{
    ...
    const {accessToken, refreshToken} = jwtToken.generateAccessTokenAndRefreshToken(user.id, user.email, user.name);
  ...
  handleAccessTokenRefresh:RequestHandler = asyncHandler(async(req:Request, res:Response)=> {
    ...
    const { accessToken, refreshToken } =
      jwtToken.generateAccessTokenAndRefreshToken(user.id, user.email, user.name);
